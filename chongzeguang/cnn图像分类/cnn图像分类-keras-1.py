...
# 搭建CNN模型
inputs = Input(shape=(x_train.shape[1:])) # 输入


x = Conv2D(32, (3, 3), padding='same')(inputs) # 2D卷积，很适合处理图像输入
x = Activation('relu')(x) # relu激活
x = Conv2D(32, (3, 3))(x) # 2D卷积
x = Activation('relu')(x) # relu激活
x = MaxPooling2D(pool_size = (2, 2))(x)  #最大池化
x = Dropout(0.25)(x) # dropout减少过拟合

x = Conv2D(64, (3, 3), padding='same')(x) # 2D卷积
x = Activation('relu')(x) # relu激活
x = Conv2D(64, (3, 3), padding='same')(x) # 2D卷积
x = Activation('relu')(x) # relu激活
x = MaxPooling2D(pool_size = (2, 2))(x)  # 最大池化
x = Dropout(0.25)(x)  # droopout减少过拟合

x = Flatten()(x) # Flatten层用来将输入“压平”，即把多维的输入一维化，常用在从卷积层到全连接层的过渡
x = Dense(512, activation='relu')(x) # 为model添加Dense层，即全链接层，512为输出
x = Dropout(0.5)(x) # droopout减少过拟合
outputs = Dense(num_classes, activation='softmax')(x) # 10维输出，通过softmax激励函数进行分类
