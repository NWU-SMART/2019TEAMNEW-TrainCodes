...
# 加入噪声数据
noise_factor = 0.5
X_train_noisy = X_train + noise_factor * np.random.normal(loc=0.0, scale=1.0, size=X_train.shape) # 正态分布
X_test_noisy = X_test + noise_factor * np.random.normal(loc=0.0, scale=1.0, size=X_test.shape)

X_train_noisy = np.clip(X_train_noisy, 0., 1.) # 将数组中的元素限制在0-1之间
X_test_noisy = np.clip(X_test_noisy, 0., 1.)

X_train = Variable(torch.from_numpy(X_train)).float()# 数据变为variable类型
X_test = Variable(torch.from_numpy(X_test)).float()

X_train_noisy = Variable(torch.from_numpy(X_train_noisy)).float()# 噪声数据变为variable类型
X_test_noisy = Variable(torch.from_numpy(X_test_noisy)).float()

X_train =X_train.permute(0,3,2,1) # 改变数据channel位置，否则输入模型报错,变成1x28x28
X_test =X_test.permute(0,3,2,1)

X_train_noisy =X_train_noisy.permute(0,3,2,1) # 改变噪声数据channel位置，否则输入模型报错,变成1x28x28
X_test_noisy =X_test_noisy.permute(0,3,2,1)
#  -------------------------- 2、数据载入与预处理 -------------------------------
...
for epoch in range(5):
    output = model(X_train_noisy)# 输入噪声数据，获取输出
