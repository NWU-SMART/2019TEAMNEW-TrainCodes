...
# myModel.add_module('hidden1',torch.nn.Linear(784,128))
# myModel.add_module('relu',torch.nn.ReLU())
# myModel.add_module('hidden2',torch.nn.Linear(128,64))
# myModel.add_module('relu',torch.nn.ReLU())
# myModel.add_module('hidden3',torch.nn.Linear(64,128))
# myModel.add_module('relu',torch.nn.ReLU())
# myModel.add_module('output',torch.nn.Linear(128,784))
...
for t in range(Epoch):
    ...
    if (t + 1) % 5 == 0:
        torch.save(model.state_dict(), "./pytorch_MultipleLayerAutoEncoder_model.pkl")  # 每5个epoch保存一次模型
        print("save model")
