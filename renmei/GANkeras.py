...
model =Sequential()
model.add(Dense(input_dim=100,output_dim=nch*14*14))
model.add(BatchNormalization())
model.add(Activation('relu'))
model.add(Reshape(nch*14*14),input_dim=nch*14*14)
...
class SimpleGan(keras.Model):
    def __init__(self, use_bn=False, use_dp=False, num_classes=10):
       super(SimpleMLP, self).__init__(self)
       self.use_bn = use_bn
       self.use_dp = use_dp
       self.dense1=keras.layers.Dense(nch*14*14,activation='relu)
       if self.use_dp:
       self.dp = keras.layers.Dropout(dropout_rate)
       if self.use_bn:
       self.bn = keras.layers.BatchNormalization(axis=-1)
       self.reshape=keras.core.reshape(output_dim=nch*14*14,input_dim=nch*14*14)
       self.sample=keras.layers.convolutional.UpSampling2D(2)
       self.conv2d1=keras.layers.Conv2D(100,(3,3),padding='same',activation='relu')
       self.conv2d2=keras.layers.Conv2D(10,(1,1),padding='same',activation='sigmoid')
       def forward(self,input)
          ...
          x=self.conv2d1(x)
          x=self.use_dp(x)
          generator=self.conv2d2(x)
       return generator
generator1=SimpleGan()
generator1.compile(loss='binary_crossentropy', optimizer=opt)
generator1.summary()
...
`
model2 =Sequential()
model2.add(Conv2D(256, (5, 5), activation="relu", strides=(2, 2), padding="same",input_dim=shp,))
model2.add(Activation('relu'))
model2.add.(Dropout(dropout_rate))
model2.add(Conv2D(512, (5, 5), activation="relu", strides=(2, 2)))
model2.add(Activation('relu'))
...
model2.add(Dense(2))
model.add(Activation('sigmoid'))
model2.compile(loss='categorical_crossentropy', optimizer=dopt)
model2.summary()


class  SimpleGan2(keras.Model):
    def __init__(self, use_bn=False, use
       super(SimpleMLP, self).__init__(self)
       self.use_bn = use_bn
       self.use_dp = use_dp
       self.conv2d1=keras.layers.Conv2D(256, (5, 5), activation="relu", strides=(2, 2), padding="same",activation='relu)
       if if self.use_dp:
       self.dp = keras.layers.Dropout(dropout_rate)
       if self.use_bn:
       self.bn = keras.layers.BatchNormalization(axis=-1)
       self.conv2d2=keras.layers.Conv2D（512, (5, 5), activation="relu", strides=(2, 2)，activation='relu')
       self.dense1=keras.layers.Flatten()
       self.dense2=keras.layers.Dense(512,activtion='relu')
       self.dense3=keras.layers.Dense(256,activtion='relu')
       self.dense4=keras.layers.Dense(2,activtion='sigmoid')
       def forward(self,input)
