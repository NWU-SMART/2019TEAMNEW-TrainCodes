...
model_name = 'keras_fashion_trained_model_API.h5'
...
input = Input(shape=x_train.shape[1:]) # 指出图像大小
x = Conv2D(32,(3,3),padding='same',activation='relu')(input) # 32,(3,3)是卷积核数量和大小
...
plt.savefig('API_cnn_valid_acc.png')
plt.show()
...
plt.xlabel('Epoch')
plt.legend(['Train','Valid'],loc='upper left')
plt.savefig('API_cnn_valid_loss.png')
plt.show()
# 训练精度可达到85%，验证精度可达到87%以上
#  -------------------------- 5、训练可视化 ------------------------------------------
