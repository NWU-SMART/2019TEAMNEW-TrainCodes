...
# 搭建CNN模型
input2 = Input(shape=(x_train.shape[1:])) # 输入

class CNN(keras.Model):
    def __init__(self):
        super(CNN, self).__init__()
        self.Conv1 = keras.layers.Conv2D(32, (3, 3), padding='same') # 2D卷积，很适合处理图像输入
        self.relu1 = keras.layers.Activation('relu') # relu激活
        self.Conv2 = keras.layers.Conv2D(32, (3, 3), padding='same') # 2D卷积
        self.relu2 = keras.layers.Activation('relu') # relu激活
        self.maxpooling1 = keras.layers.MaxPooling2D(pool_size=(2, 2)) #最大池化
        self.dropout1 = keras.layers.Dropout(0.25) # dropout减少过拟合
        ...
        self.Flatten = keras.layers.Flatten() # Flatten层用来将输入“压平”，即把多维的输入一维化，常用在从卷积层到全连接层的过渡
        self.dense1 = keras.layers.Dense(512, activation='relu')
        self.dropout3 = keras.layers.Dropout(0.5)
        self.dense2 = keras.layers.Dense(10, activation='softmax') # 10维输出，通过softmax激励函数进行分类
...
if not data_augmentation: # 图像增强
    ...
    history = model.fit_generator(datagen.flow(x_train, y_train,  # 按batch_size大小从x,y生成增强数据
                        ...
                        workers=6  # 在使用基于进程的线程时，最多需要启动的进程数量。
                       )
