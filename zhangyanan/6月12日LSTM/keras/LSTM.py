...
# 把所有评论都填充到同样长度
max_len = 200
x_train = keras.preprocessing.sequence.pad_sequences(x_train, maxlen=max_len)  # 填充到300
x_test = keras.preprocessing.sequence.pad_sequences(x_test, maxlen=max_len)


#  ---------------------------------- 构建模型 --------------------------------------
model=Sequential()
# 用长度为16的向量表示这10000个单词,10000是输入维度，16是embedding维度，200是每个序列长度
...
# 记录训练过程
history = model.fit(x_train, y_train, epochs=5, batch_size=2000, validation_data=(x_test, y_test))
