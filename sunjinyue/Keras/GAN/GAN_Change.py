...
'''
     sss 程序代码图像形状备注修改
     sss 理解训练过程
     sss 改变最后epoch次数为100 batch为10 减少循环次数
'''
...
# Optim优化器
opt = Adam(lr=1e-4)     # sss 生成器优化器
dopt = Adam(lr=1e-5)    # sss 判别器优化器
...
K.image_data_format() == 'channels_first'      # sss返回默认图像的维度顺序 还有一个channels_last   Keras中默认为channels_last



# 生成1 * 28 * 28的图片     sss28*28*1？？？？？？？？
nch = 200
...
# 39200 --> 200 * 14 * 14
H = Reshape( [ 14, 14,nch] )(H)  # 转成200 * 14 * 14  sss14*14*200
print(H.shape)

# 上采样 200 * 14 * 14 --> 200 * 28 * 28    sss 28*28*200
H = UpSampling2D(size=(2, 2))(H)

# 200 * 28 * 28 --> 100 * 28 * 28       sss 28*28*100
H = Convolution2D(100, (3, 3), padding="same", kernel_initializer='glorot_normal')(H)
H = BatchNormalization()(H)
H = Activation('relu')(H)

# 100 * 28 * 28 --> 50 * 28 * 28        sss 28*28*50
H = Convolution2D(50, (3, 3), padding="same", kernel_initializer='glorot_normal')(H)
H = BatchNormalization()(H)
H = Activation('relu')(H)

# 50 * 28 * 28 --> 1 * 28 * 28          sss28*28*1
...
# 1 * 28 * 28 --> 256 * 14 * 14, 权重参数 (28-5+1) * 256 = 6656      sss 14*14*256
H = Convolution2D(256, (5, 5), activation="relu", strides=(2, 2), padding="same")(d_input)
H = LeakyReLU(0.2)(H)
H = Dropout(dropout_rate)(H)

# 256 * 14 * 14 --> 512 * 7 * 7       sss 7*7*512
...
# 构造GAN
gan_input = Input(shape=[100])  # 输入数据     sss 输入一百维向量
H = generator(gan_input)  # 生成新的图像，生成器  1 * 100 --> 1 * 28 * 28     sss  28*28*1
gan_V = discriminator(H)  # 判别器  1 * 28 * 28  --> 1 * 2 (输出 [0 1]为真实图像, [1 0]为生成图像)
...
print(X_train.shape) # (60000, 1, 28, 28)   sss 60000，28*28*1
print(XT.shape) # (10000, 1, 28, 28)        sss 10000，28*28*1
...
X = np.concatenate((XT, generated_images))      #sss np.concatenate（） 数组拼接
n = XT.shape[0]
