...
# 打印10张测试集手写体的压缩效果
n = 10
plt.figure(figsize=(20, 8),facecolor='blue')       # sss 图片显示加以底色蓝色
for i in range(n):
    ax = plt.subplot(1, n, i+1)    # sss 绘制图像 1行n列
    plt.imshow(encoded_imgs[i].reshape(4, 16).T)  # 8*8 的特征，转化为 4*16的图像   如果不加.T则显示为横的 
    plt.gray()
    ax.get_xaxis().set_visible(False)    #  sss 不显示x坐标轴
    ax.get_yaxis().set_visible(False)    #  sss 不显示y坐标轴
plt.show()
