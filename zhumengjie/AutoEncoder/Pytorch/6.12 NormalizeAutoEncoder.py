...
#  -------------------------- 4、模型训练 -------------------------------
# L2正则化，torch.optim集成的优化器只有L2正则化方法
...
print("-----------训练结束-----------")
torch.save(model.state_dict(), "torch_SAutoEncode.pkl")  # 保存模型参数
# -------------------------------模型训练------------------------

#  -------------------------- 5、模型测试 -------------------------------
print("-----------测试开始-----------")

model.load_state_dict(torch.load('torch_SAutoEncode.pkl'))  # 加载训练好的模型参数
for i in range(epoch):
    # 预测结果
    pred = model(x_test)
    # 计算损失
