...
class DataManager(object):
    ...
    def __init__(
        ...
        transform_tr: train transform
        transform_te: test transform
        '''
...
class ImageDataManager(DataManager):
    ...
    def __init__(
        ...
        for name in self.sources:
            # Initializes an image dataset，如果源训练集不存在则返回提示信息
            trainset_ = init_image_dataset(
                name,
                transform=self.transform_tr,  # transform_tr: train transform
                k_tfm=k_tfm,                  # number of times to apply augmentation to an image
                mode='train',
                combineall=combineall,        # 将train、query和gallery组合在一个数据集中进行训练,False
                root=root,
                split_id=split_id,            # 拆分id
                cuhk03_labeled=cuhk03_labeled,  # use cuhk03 labeled images.
                cuhk03_classic_split=cuhk03_classic_split,  # use the classic split in cuhk03.
                market1501_500k=market1501_500k         # add 500K distractors to the gallery set in market1501. Default is False.
            )
            trainset.append(trainset_)   # Append trainset_ to the end of the trainset
        trainset = sum(trainset)         # 相加得到最终的trainset

        self._num_train_pids = trainset.num_train_pids  # Returns the number of training person identities
        self._num_train_cams = trainset.num_train_cams  # Returns the number of training cameras

        # trainset加载
        self.train_loader = torch.utils.data.DataLoader(
            trainset,
            sampler=build_train_sampler   # Builds a training sampler
            (trainset.train,
             train_sampler,
