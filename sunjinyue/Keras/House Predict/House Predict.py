...
# 转成DataFrame格式方便数据处理   sss DataFrame格式可理解为一张表
...
#  -------------------------- 数据归一化处理 -------------------------------
# sss 训练集归一化 归一化可以减少量纲不同带来的影响，使得不同特征之间具有可比性；
...
#                  activity_regularizer=regularizers.l1(0.01),  # sss希望这层输出尽量小施加在输出上的正则项 L1正则  根据公式添加上w平方之和
...
# sss API方法输入为一个张量输出为张量
# from keras.layers import Input
# from keras.models import Model
#
# inputs = Input(shape=(x_train_pd.shape[1],))
# x = Dense(units=10, activation='relu')(inputs)  # sss  Dense定义为一个函数 输入为inputs 将其作为此函数的输入 得到的结果张量给x
# x = Dropout(0.2)(x)
# x = Dense(units=15, activation='relu')(x)
# outputs = Dense(units=1, activation='linear')(x)
# model = Model(inputs=inputs, outputs=outputs)    #sss Model类将输入张量和输出张量转化为一个模型
...
class HouseModel(keras.Model):  # 继承keras.Model
    def __init__(self):   # sss 网络层定义在def __init__(self，...)中  定义网络层中所包含的内容
    ...
    def call(self, inputs):  # sss 前向传播在def call(self, inputs)中定义  调用网络层中各部分 并加以输入
