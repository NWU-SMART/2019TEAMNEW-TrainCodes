...
X_train = [cv2.cvtColor(cv2.resize(i, (48, 48)), cv2.COLOR_GRAY2RGB) for i in x_train]    # sss cv2.cvtColor()颜色空间转换函数
...
# weights='imagenet'表示使用VGG16的参数
path = 'D:\\应用软件\\研究生学习\\vgg16_weights_tf_dim_ordering_tf_kernels_notop.h5'    # sss 相对路径读取VGG16无全连接层权重初始值
base_model = applications.VGG16(include_top=False, weights=path, input_shape=x_train.shape[1:])  # 第一层需要指出图像的大小
...
opt = keras.optimizers.rmsprop(lr=0.0001, decay=0.000001)  # RMSprop优化器
